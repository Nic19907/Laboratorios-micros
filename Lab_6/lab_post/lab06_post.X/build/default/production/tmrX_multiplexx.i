...
restart_tmr1 macro
    ...
    bcf ((PIR1) and 07Fh), 0 ;limpiar la bandera
    endm
...
config_clock:
    ...
    bcf ((OSCCON) and 07Fh), 4 ;0 oscilador a 4MHz
    bsf ((OSCCON) and 07Fh), 0 ;se utiliza el oscilador como el reloj interno
return

config_tmr0:
    banksel OPTION_REG
    bcf ((OPTION_REG) and 07Fh), 5 ;reloj interno
    bcf ((OPTION_REG) and 07Fh), 3 ;prescaler asignado al TMR0
    bcf ((OPTION_REG) and 07Fh), 2 ;0
    bsf ((OPTION_REG) and 07Fh), 1 ;1
    bcf ((OPTION_REG) and 07Fh), 0 ;0 010, escala de 1:8
    return

config_tmr1:
    banksel PORTA
    bcf ((T1CON) and 07Fh), 6 ;always active
    bcf ((T1CON) and 07Fh), 5 ;0
    bsf ((T1CON) and 07Fh), 4 ;1, 01, escala 1:2
    bcf ((T1CON) and 07Fh), 3 ;LP oscilador apagado
    bcf ((T1CON) and 07Fh), 1 ;Usa el oscilador interno de PIC
    bsf ((T1CON) and 07Fh), 0 ;prender el tmr1
    restart_tmr1
    return
...
config_tmr2: ;se le pone el valor al que el contador debe llegar
    ...
    bsf ((T2CON) and 07Fh), 3 ;1, 1111 Postscaler 1:16

    bsf ((T2CON) and 07Fh), 1 ;1
    bsf ((T2CON) and 07Fh), 0 ;1, 11 prescaler 1:16
...
config_ie:
    ...
    bsf ((INTCON) and 07Fh), 5 ;interrupcion tmr1

    bcf ((INTCON) and 07Fh), 2 ;bandera tmr0
