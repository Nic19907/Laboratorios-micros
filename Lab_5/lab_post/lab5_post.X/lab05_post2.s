...
show:
    movf    display_7dec, w
    clrf    PORTD	;borrar portd por si acaso
    movwf   PORTB	;pasar el valor de centenas al portb
    bsf	    PORTD, 0	;activar el display 1
    call    delay_small	;delay para que se vea
    bcf	    PORTD, 0	;apagar el display 1
    
    movf    display_7dec+1, w
    clrf    PORTD	;borrar portd por si acaso
    movwf   PORTB	;pasar el valor de centenas al portb
    bsf	    PORTD, 1	;activar el display 1
    call    delay_small	;delay para que se vea
    bcf	    PORTD, 1	;apagar el display 1
    
    movf    display_7dec+2, w
    clrf    PORTD	;borrar portd por si acaso
    movwf   PORTB	;pasar el valor de centenas al portb
    bsf	    PORTD, 2	;activar el display 1
    call    delay_small	;delay para que se vea
    bcf	    PORTD, 2	;apagar el display 1
    return
div_100:
    movf    PORTA, W
...
delay_small:
    movlw   25		;valor inicial contador 
    movwf   time_3
    decfsz  time_3, 1	;decrementar por 1 el contador 
    goto    $-1	
    return
