...
isr: ;codigo de interrupcion
    ...
    btfsc ((INTCON) and 07Fh), 2 ;ver si la bandera del tmr0 esta prendida
...
main:
    call config_io ;PORTA salida ; ((PORTB) and 07Fh), 0 y ((PORTB) and 07Fh), 1 inputs
...
config_clock:
    banksel OSCCON
    bsf ((OSCCON) and 07Fh), 6 ;1 ;1MHz
    bsf ((OSCCON) and 07Fh), 5 ;1
    bsf ((OSCCON) and 07Fh), 4 ;1 oscilador a 8MHz
...
config_tmr0_ie:
    ...
    bsf ((OPTION_REG) and 07Fh), 2 ;0
    bsf ((OPTION_REG) and 07Fh), 1 ;0
    bsf ((OPTION_REG) and 07Fh), 0 ;0 000, escala de 1:2
    banksel PORTA
    restart_tmr0
    bsf ((INTCON) and 07Fh), 7 ;interrupciones activadas
    bsf ((INTCON) and 07Fh), 5
    bcf ((INTCON) and 07Fh), 2 ;banderas del tmr0
    return
...
;-------------------------------------------------------------------------------
contador_8bits:
    btfsc ((PORTE) and 07Fh), 0 ;Push button en Pull DOWN
    call inc_A
    btfsc ((PORTE) and 07Fh), 1 ;lo mismo que ((PORTD) and 07Fh), 0
    call dec_A
...
inc_A:
    ...
    btfsc ((PORTE) and 07Fh), 0 ;(el boton esta ciendo precionado) hasta que no se
...
dec_A:
    call delay_big ;esta para evitar un rebote
    btfsc ((PORTE) and 07Fh), 1 ;(el boton esta ciendo precionado) hasta que no se
       ;suelte el boton no saltara el goto
    goto $-1
